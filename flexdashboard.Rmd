...
rect <- data.frame(
  ...
  ymin = rep(-Inf, 4),
  ymax = rep(Inf, 4),
  fill=factor(c("black", "blue", "black", "blue"))
)
...
map_IHDI <- africa_data %>%  ggplot() + 
  ...
  scale_fill_gradientn("IHDI", colors=c("black", "#E64814", "#E9CB0D", "#9CDE3B"),
                       limits = limits) +
...
ratio_graph <- africa_data %>% drop_na(Inequality.adjusted.HDI..IHDI.) %>% 
  ...
  scale_fill_manual("Classes IDH", values=c("#9CDE3B","#EAC60E", "#EA7513","#BE3E14"),
  ...
  geom_rect(xmin=-Inf, xmax=Inf, ymin=37, ymax=42.5, color="black", alpha=0, size=0.1) +
